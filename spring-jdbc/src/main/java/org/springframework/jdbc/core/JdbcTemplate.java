...
public class JdbcTemplate extends JdbcAccessor implements JdbcOperations {
		...
		return execute(psc, new PreparedStatementCallback<T>() {
			...
			public T doInPreparedStatement(PreparedStatement ps) throws SQLException {
				...
				try {
					...
					// 执行查询
					rs = ps.executeQuery();
					// 将结果进行封装并转换至POJO。rse当前代表的类为RowMapperResultSetExtractor
					return rse.extractData(rs);
				} finally {
					JdbcUtils.closeResultSet(rs);
	 ...
	 * <p>
	 * 准备给定的JDBC语句（或PreparedStatement或CallableStatement），应用诸如获取大小，最大行数和查询超时之类的语句设置。
	 *
	...
	protected void applyStatementSettings(Statement stmt) throws SQLException {
		// fetchSize是为了减少网络交互次数设计的。
		// 访问ResultSet时，如果它每次只从服务器上读取一行数据，则会产生大量的开销。fetchSize是当调用rs.next时，ResultSet会一次性
		// 从服务器上取得多少行数据回来，这样在下次rs.next时，它可以直接从内存中获取数据而不需要网络交互。
		int fetchSize = getFetchSize();
		if (fetchSize != -1) {
			stmt.setFetchSize(fetchSize);
		}

		// 将Statement对象生成的所有ResultSet对象可以包含的最大行数限制设置为给定数
	 ...
	 * <p>
	 * 如果我们不忽略警告，则抛出SQLWarningException， 否则，在调试级别记录警告。
	 *
	...
	protected void handleWarnings(Statement stmt) throws SQLException {
		...
		if (isIgnoreWarnings()) {
			// 如果日志开启的情况下打印日志
			if (logger.isDebugEnabled()) {
				// 提供有关数据库访问警告信息的异常。这些警告直接链接到导致报告警告的方法所在的对象。
				// 警告可以从Connection、Statement和ResultSet对象中获得。试图在已经关闭的连接上获取警告将导致抛出异常。
