 ...
 * <p>
 * 在PreparedStatement上运行的代码的通用回调接口。
 * 允许在单个PreparedStatement上执行任意数量的操作，
 * 例如，单个{@code executeUpdate}调用或重复的{@code executeUpdate}调用（具有不同的参数）。
 *
...
public interface PreparedStatementCallback<T> {
	 ...
	 * <p>
	 * 由{@code JdbcTemplate.execute}使用活动的JDBC PreparedStatement进行调用。
