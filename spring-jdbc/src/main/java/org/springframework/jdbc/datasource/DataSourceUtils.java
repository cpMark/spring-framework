...
public abstract class DataSourceUtils {
	 ...
	 * <p>
	 * 实际上是从给定的DataSource获得JDBC连接。 与{@link #getConnection}相同，但会引发原始SQLException。
	 *
	 * <p>Is aware of a corresponding Connection bound to the current thread, for example
	 * when using {@link DataSourceTransactionManager}. Will bind a Connection to the thread
	 * if transaction synchronization is active (e.g. if in a JTA transaction).
	 * <p>
	 * 知道绑定到当前线程的相应连接，例如，在使用{@link DataSourceTransactionManager}时。
	 * 如果事务同步处于活动状态（例如，在JTA事务中），则将Connection连接绑定到线程。
	 *
	 ...
	 * <p>
	 * 如果没有外部管理（即未绑定到线程），则关闭从给定的DataSource获得的给定的Connection。
	 *
	...
	public static void doReleaseConnection(@Nullable Connection con, @Nullable DataSource dataSource) throws SQLException {
		...
		if (dataSource != null) {
			// 当前线程存在事务的情况下说明存在共用数据库连接，直接使用ConnectionHolder中的released方法进行连接减一而不是真正的释放连接
			ConnectionHolder conHolder = (ConnectionHolder) TransactionSynchronizationManager.getResource(dataSource);
