 ...
 * <p>
 * 一个bean定义的扫描器，它检测类路径上的bean候选者，用给定的注册表（{@code BeanFactory}或{@code ApplicationContext}）注册相应的bean定义。
 *
		...
		for (String basePackage : basePackages) {
			...
			for (BeanDefinition candidate : candidates) {
				...
				if (candidate instanceof AnnotatedBeanDefinition) {
					// 如果是AnnotatedBeanDefinition类型的bean，需要检测下常用注解。如：Primary、Lazy等
