...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	protected EntityResolver getEntityResolver() {
		if (this.entityResolver == null) {
			...
			// 如果本身ResourceLoader对象不为空则包装成ResourceEntityResolver对象，否则包装成DelegatingEntityResolver对象
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		// 查询当前线程正在加载的资源信息，如果容器为空则新增，并保存到当前线程的ThreadLocal中
		...
		try {
			// 从EncodedResource对象中获取其包装的Resource对象，然后获取输入流
			InputStream inputStream = encodedResource.getResource().getInputStream();
			try {
				InputSource inputSource = new InputSource(inputStream);
				// 如果EncodedResource对象设置了字符集，则保存到InputSource对象中
				if (encodedResource.getEncoding() != null) {
					inputSource.setEncoding(encodedResource.getEncoding());
				}
				// 正在加载BeanDefinition的地方
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
		...
		try {
			...
			//注册文档对象，并返回本次注册的bean definitions的总数
	...
	protected Document doLoadDocument(InputSource inputSource, Resource resource) throws Exception {
		//documentLoader是一个DefaultDocumentLoader实例
	 ...
	 * <p>
	 * 创建解析器类{@link DefaultBeanDefinitionDocumentReader}的新实例，并在其上调用{@code registerBeanDefinitions}。
	 *
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		...
		//在注册新的文档对象到注册器之前，获取之前注册的bean definition数量
		//默认使用的注册器是DefaultListableBeanFactory的子类，当前我们编写的示例为XmlBeanFactory
		int countBefore = getRegistry().getBeanDefinitionCount();
		//注册doc到bean definition容器中
		documentReader.registerBeanDefinitions(doc, createReaderContext(resource));
		//返回本次加载的bean definition个数
	 ...
	 * <p>
	 * 如果之前未设置，默认使用DefaultNamespaceHandlerResolver对象
	 *
