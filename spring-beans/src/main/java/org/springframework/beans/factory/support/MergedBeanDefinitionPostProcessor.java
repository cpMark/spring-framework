 ...
 * <p>
 * 在运行时用于合并的Bean定义的后处理器回调接口。{@link BeanPostProcessor}实现类可以实现此子接口，
 ...
 * <p>
 * {@link #postProcessMergedBeanDefinition}方法可能会进行内省bean定义，以便在后处理bean的实际实例之前准备一些缓存的元数据。
 * 还允许修改bean定义，但是仅用于实际上用于并发修改的定义属性。本质上，这仅适用于在{{@link RootBeanDefinition}上定义的操作，而不适用于其基类的属性
 *
