...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	 ...
	 * 即它不会被考虑作为其他bean自动状态的候选者，但是该bean本身还是可以使用自动装配来注入其他bean的
	 * <p>
	...
	/**
	 * 是否以一种宽松的模式解析构造函数，默认为true，如果为false则在如下情况：
	 * interface ITest{
	 * <p>
	 * }
	 * <p>
	 ...
	 * }
	 * }
	 * 抛出异常，因为Spring无法准确定位哪个构造函数，程序设置
	 */
	private boolean lenientConstructorResolution = true;

	/**
	 * 对应bean属性factory-bean，用法：
	 * <bean id="instanceFactoryBean" class="com.example.demo.InstanceFactoryBean"/>
	 * <bean id="test" factory-bean="instanceFactoryBean" factory-method="createTestInstance"/>
	 */
	@Nullable
	 ...
	 * <p>
	 * 使用给定的BeanDefinition来重写当前BeanDefinition的设置
	 *
	 * <ul>
	...
	public void overrideFrom(BeanDefinition other) {
		// 使用给定的BeanDefinition的beanNameClass、scope、abstract、lazyInit、FactoryBeanName、factoryMethodName、
		// role、source、来覆盖当前BeanDefinition
		...
		if (other instanceof AbstractBeanDefinition) {
			// 如果给定的BeanDefinition是一个AbstractBeanDefinition，则进行强转，并使用其特有属性来覆盖值
	 ...
	 * <p>
	 * 返回此bean是否是抽象的，即本身不打算实例化，而是仅仅用作具体子BeanDefinition的父级
	 */
	@Override
	 ...
	 * <p>
	 * 返回此BeanDefinition是否是合成的，即不是由应用程序本身定义的
	 */
	...
	public void validate() throws BeanDefinitionValidationException {
		// 如果存在方法重写，且存在对应bean属性的factory-method，则抛出异常
	 ...
	 * <p>
	 * 验证并准备为当前bean中定义的方法进行重写
	 *
	 * @throws BeanDefinitionValidationException in case of validation failure
	 */
	public void prepareMethodOverrides() throws BeanDefinitionValidationException {
		// Check that lookup methods exist and determine their overloaded status.
		// 检查是否存在方法重写，即lookup-method和replace-method
	...
	protected void prepareMethodOverride(MethodOverride mo) throws BeanDefinitionValidationException {
		//通过反射获取对应类中对应方法名的个数——包含本类+所有接口+所有父类中所有同名的方法个数
		...
		if (count == 0) {
			...
			// 设置覆盖的方法没有重载（即不需要参数类型匹配以消除同名方法的歧义）
